//
// Fl_Scroll_New widget for the Fast Light Tool Kit (FLTK).
//
...
#include <FL/Fl_Scroll_New.H>
#include <FL/fl_draw.H>

/** Clear all but the scrollbars... */
void Fl_Scroll_New::clear() {
  Fl_Group::clear();
}
...
/**
  ...
  \retval     2   not possible

  \see Fl_Group::delete_child(int index)

  \since FLTK 1.4.0
*/
int Fl_Scroll_New::delete_child(int index) {
...
//
void Fl_Scroll_New::draw_clip(void* v,int X, int Y, int W, int H) {
  fl_push_clip(X,Y,W,H);
  auto s = (Fl_Scroll_New *)v;
  ...
  switch (s->box()) {
    ...
    case _FL_PLASTIC_DOWN_FRAME :
        if (s->parent() == (Fl_Group *)s->window() && Fl::scheme_bg_) {
          auto bg_image = (Fl_Tiled_Image *)Fl::scheme_bg_;
          auto image = bg_image->image();
          bg_image->draw(X - (X % image->w()),
                         Y - (Y % image->h()),
                         W + image->w(),
                         H + image->h());
          break;
...
/**
  ...
  \see bbox()
*/
void Fl_Scroll_New::recalc_scrollbars(ScrollInfo &si) const {
  ...
  // See if children would fit if we had no scrollbars...
  {
    ...
    if (type() & VERTICAL) {
      if ((type() & ALWAYS_ON) || si.child.t < Y || si.child.b > Y+H) {
        si.vneeded = 1;
        W -= si.scrollsize;
        if (align & FL_ALIGN_LEFT) X += si.scrollsize;
      }
    }
    if (type() & HORIZONTAL) {
      if ((type() & ALWAYS_ON) || si.child.l < X || si.child.r > X+W) {
        si.hneeded = 1;
        H -= si.scrollsize;
        if (align & FL_ALIGN_TOP) Y += si.scrollsize;
  ...
  si.hscroll.y = (scrollbar_v()->align() & FL_ALIGN_TOP)
                     ? si.innerbox.y
  ...
  si.vscroll.x = (scrollbar_v()->align() & FL_ALIGN_LEFT)
                     ? si.innerbox.x
...
*/
void Fl_Scroll_New::bbox(int& X, int& Y, int& W, int& H) const {
  ScrollInfo si;
...
void Fl_Scroll_New::draw() {
  ...
  // Calculate where scrollbars should go, and draw them
  {
      ...
      if (si.vneeded && !scrollbar_v()->visible()) {
        scrollbar_v()->set_visible();
        d = FL_DAMAGE_ALL;
      }
      else if (!si.vneeded && scrollbar_v()->visible()) {
        scrollbar_v()->clear_visible();
        draw_clip(this, si.vscroll.x, si.vscroll.y, si.vscroll.w, si.vscroll.h);
        d = FL_DAMAGE_ALL;
      }
      if (si.hneeded && !scrollbar_h()->visible()) {
        scrollbar_h()->set_visible();
        d = FL_DAMAGE_ALL;
      }
      else if (!si.hneeded && scrollbar_h()->visible()) {
        scrollbar_h()->clear_visible();
        draw_clip(this, si.hscroll.x, si.hscroll.y, si.hscroll.w, si.hscroll.h);
        d = FL_DAMAGE_ALL;
      }
      else if ( scrollbar_h()->h() != si.scrollsize || scrollbar_v()->w() != si.scrollsize ) {
         // scrollsize changed
         d = FL_DAMAGE_ALL;
      }

      scrollbar_v()->resize(si.vscroll.x, si.vscroll.y, si.vscroll.w, si.vscroll.h);
      oldy = yposition_ = si.vscroll.pos;       // si.innerchild.y - si.child.t;
      scrollbar_v()->value(si.vscroll.pos, si.vscroll.size, si.vscroll.first, si.vscroll.total);

      scrollbar_h()->resize(si.hscroll.x, si.hscroll.y, si.hscroll.w, si.hscroll.h);
      oldx = xposition_ = si.hscroll.pos;       // si.innerchild.x - si.child.l;
      scrollbar_h()->value(si.hscroll.pos, si.hscroll.size, si.hscroll.first, si.hscroll.total);
  }

  // draw the scrollbars:
  if (d & FL_DAMAGE_ALL) {
    draw_child(*scrollbar_v());
    draw_child(*scrollbar_h());
    if (scrollbar_v()->visible() && scrollbar_h()->visible()) {
      // fill in the little box in the corner
      fl_color(color());
      fl_rectf(scrollbar_v()->x(), scrollbar_h()->y(), scrollbar_v()->w(), scrollbar_h()->h());
    }
  } else {
    update_child(*scrollbar_v());
    update_child(*scrollbar_h());
  }
}

/**
  Resizes the Fl_Scroll_New widget and moves its children if necessary.

  The Fl_Scroll_New widget first resizes itself, and then it moves all its
  children if (and only if) the Fl_Scroll_New widget has been moved. The
  children are moved by the same amount as the Fl_Scroll_New widget has been
  moved, hence all children keep their relative positions.

  \note Fl_Scroll_New::resize() does \b not call Fl_Group::resize(), and
  child widgets are \b not resized.

  Since children of an Fl_Scroll_New are not resized, the resizable() widget
  is ignored (if it is set).

  The scrollbars are moved to their proper positions, as given by
  Fl_Scroll_New::scrollbar_v()->align(), and switched on or off as necessary.

  \note Due to current (FLTK 1.3.x) implementation constraints some of this
  may effectively be postponed until the Fl_Scroll_New is drawn the next time.
  This may change in a future release.

  \sa Fl_Group::resizable()
  \sa Fl_Widget::resize(int,int,int,int)
*/
void Fl_Scroll_New::resize(int X, int Y, int W, int H) {
  int dx = X-x(), dy = Y-y();
  int dw = W-w(), dh = H-h();
  ...
  if (dw==0 && dh==0) {
    char pad = ( scrollbar_v()->visible() && scrollbar_h()->visible() );
    char al = ( (scrollbar_v()->align() & FL_ALIGN_LEFT) != 0 );
    char at = ( (scrollbar_v()->align() & FL_ALIGN_TOP)  !=0 );
    scrollbar_v()->position(al?X:X+W-scrollbar_v()->w(), (at&&pad)?Y+scrollbar_h()->h():Y);
    scrollbar_h()->position((al&&pad)?X+scrollbar_v()->w():X, at?Y:Y+H-scrollbar_h()->h());
  } else {
...
/**  Moves the contents of the scroll group to a new position.
  ...
  This is like moving the scrollbars of the Fl_Scroll_New around. For instance:
  \code
    Fl_Scroll_New scroll (10,10,200,200);
  ...
  relative to the origin of the Fl_Scroll_New (10,10), i.e. Fl_Box b3 will
  be visible in the top left corner of the scroll area.
*/
void Fl_Scroll_New::scroll_to(int X, int Y) {
  int dx = xposition_-X;
  int dy = yposition_-Y;
  if (!dx && !dy) return;
  xposition_ = X;
...
void Fl_Scroll_New::hscrollbar_cb(Fl_Widget *o, void *v) {
  Fl_Scroll_New *s = (Fl_Scroll_New*)v;
  s->scroll_to(int(((Fl_Scrollbar*)o)->value()), s->yposition());
}

void Fl_Scroll_New::vscrollbar_cb(Fl_Widget *o, void *v) {
  Fl_Scroll_New* s = (Fl_Scroll_New *)v;
  s->scroll_to(s->xposition(), int(((Fl_Scrollbar*)o)->value()));
}

/**
  Creates a new Fl_Scroll_New widget using the given position,
  size, and label string. The default boxtype is FL_NO_BOX.

  The destructor <I>also deletes all the children</I>. This allows a
  whole tree to be deleted at once, without having to keep a pointer to
  all the children in the user code. A kludge has been done so the
  Fl_Scroll_New and all of its children can be automatic (local)
  variables, but you must declare the Fl_Scroll_New <I>first</I>, so
  that it is destroyed last.
*/
Fl_Scroll_New::Fl_Scroll_New(int X,int Y,int W,int H,const char* L)
  : Fl_Group(X,Y,W,H,L) {

  Fl_Group::current(0); // don't add the scrollbars to the group
  vscrollbar_ = new Fl_Scrollbar(X + W - Fl::scrollbar_size(), Y,
                                 Fl::scrollbar_size(), H - Fl::scrollbar_size());
  hscrollbar_ = new Fl_Scrollbar(X, Y + H - Fl::scrollbar_size(),
  ...
  scrollbar_h()->callback(hscrollbar_cb, this);
  scrollbar_v()->callback(vscrollbar_cb, this);
  begin();
}

int Fl_Scroll_New::handle(int event) {
  // *** FIXME *** HACK ***
  if (event == FL_PUSH) {
    if (Fl::event_inside(scrollbar_h()) ||
        Fl::pushed() == scrollbar_h() ||
        Fl::belowmouse() == scrollbar_h()) {
      Fl::pushed(scrollbar_h());
      Fl::belowmouse(scrollbar_h());
      return scrollbar_h()->handle(event);
    }
    if (Fl::event_inside(scrollbar_v()) ||
        Fl::pushed() == scrollbar_v() ||
        Fl::belowmouse() == scrollbar_v()) {
      Fl::pushed(scrollbar_v());
      Fl::belowmouse(scrollbar_v());
      return scrollbar_v()->handle(event);
    }
  }
