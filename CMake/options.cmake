...
#######################################################################
if(APPLE)
  option(FLTK_APPLE_X11 "use X11" OFF)
...
if(WIN32 OR (APPLE AND NOT FLTK_APPLE_X11))
...
if(UNIX)
  ...
  if(NOT APPLE)
    option(FLTK_OPTION_WAYLAND "support both Wayland and X11 backends" ON)
  endif(NOT APPLE)
  if(FLTK_OPTION_WAYLAND)
    ...
    if(NOT(WLDCLIENT_FOUND AND WLDCURSOR_FOUND AND WLDPROTO_FOUND AND XKBCOMMON_FOUND))
      ...
      message(STATUS "Consequently, FLTK_OPTION_WAYLAND is set to OFF.")
      unset(FLTK_OPTION_WAYLAND CACHE)
      set(FLTK_OPTION_WAYLAND 0)
    endif(NOT(WLDCLIENT_FOUND AND WLDCURSOR_FOUND AND WLDPROTO_FOUND AND XKBCOMMON_FOUND))
  endif(FLTK_OPTION_WAYLAND)

  if(FLTK_OPTION_WAYLAND)
    option(FLTK_OPTION_WAYLAND_ONLY "support Wayland backend only" OFF)
    set(FLTK_USE_WAYLAND 1)
    if(NOT FLTK_OPTION_WAYLAND_ONLY)
      include(FindX11)
    endif(NOT FLTK_OPTION_WAYLAND_ONLY)
    if(X11_FOUND)
    ...
    unset(FLTK_OPTION_USE_XRENDER CACHE)
    unset(FLTK_OPTION_USE_XINERAMA CACHE)
    unset(FLTK_OPTION_USE_XFT CACHE)
    unset(FLTK_OPTION_USE_XCURSOR CACHE)
    unset(FLTK_OPTION_USE_XFIXES CACHE)
    if(X11_FOUND)
    ...
    unset(FLTK_OPTION_USE_PANGO CACHE)
    set(FLTK_OPTION_USE_PANGO TRUE CACHE BOOL "use lib Pango")
    ...
    if(USE_SYSTEM_LIBDECOR)
      set(FLTK_OPTION_WAYLAND_GTK ON)
    else()
      option(FLTK_OPTION_WAYLAND_GTK "Allow to use libdecor's GTK plugin" ON)
...
if(WIN32)
  option(FLTK_OPTION_USE_GDIPLUS "use GDI+ when possible for antialiased graphics" ON)
  if(FLTK_OPTION_USE_GDIPLUS)
    set(USE_GDIPLUS TRUE)
    if(NOT MSVC)
      list(APPEND FLTK_LDLIBS "-lgdiplus")
    endif(NOT MSVC)
  endif(FLTK_OPTION_USE_GDIPLUS)
endif(WIN32)

#######################################################################
option(FLTK_OPTION_USE_STD "allow FLTK to use some std:: features" OFF)
if(FLTK_OPTION_USE_STD)
  set(FLTK_USE_STD 1)
else()
  set(FLTK_USE_STD 0)
endif()
...
set(PATH_TO_XLIBS)
if((NOT APPLE OR FLTK_APPLE_X11) AND NOT WIN32 AND NOT FLTK_OPTION_WAYLAND)
  include(FindX11)
  if(X11_FOUND)
    set(FLTK_USE_X11 1)
...
endif((NOT APPLE OR FLTK_APPLE_X11) AND NOT WIN32 AND NOT FLTK_OPTION_WAYLAND)
...
endif(FLTK_APPLE_X11)

#######################################################################
option(FLTK_OPTION_USE_POLL "use poll if available" OFF)
mark_as_advanced(FLTK_OPTION_USE_POLL)

if(FLTK_OPTION_USE_POLL)
  CHECK_FUNCTION_EXISTS(poll USE_POLL)
endif(FLTK_OPTION_USE_POLL)
...
#######################################################################
option(FLTK_OPTION_USE_SVG "read/write SVG files" ON)

if(FLTK_OPTION_USE_SVG)
  set(FLTK_USE_SVG 1)
endif(FLTK_OPTION_USE_SVG)

#######################################################################
set(HAVE_GL LIB_GL OR LIB_MesaGL)

if(HAVE_GL)
  option(FLTK_OPTION_USE_GL "use OpenGL" ON)
endif(HAVE_GL)

if(FLTK_OPTION_USE_GL)
  if(FLTK_OPTION_WAYLAND)
...
if(OPENGL_FOUND)
  ...
  elseif(APPLE AND NOT FLTK_APPLE_X11)
    set(GLLIBS "-framework OpenGL")
  elseif(FLTK_OPTION_WAYLAND)
...
if(WIN32 AND NOT CYGWIN)
  ...
  set(FLTK_OPTION_USE_PTHREADS FALSE)
else()
  option(FLTK_OPTION_USE_PTHREADS "use multi-threading with pthreads" ON)
...
else(FLTK_OPTION_USE_PTHREADS)

  set(HAVE_PTHREAD_H 0)

endif(FLTK_OPTION_USE_PTHREADS)
...
if(debug_threads)
  ...
  fl_debug_var(FLTK_OPTION_USE_PTHREADS)
...
if(X11_Xinerama_FOUND)
  option(FLTK_OPTION_USE_XINERAMA "use lib Xinerama" ON)
endif(X11_Xinerama_FOUND)

if(FLTK_OPTION_USE_XINERAMA)
...
endif(FLTK_OPTION_USE_XINERAMA)

#######################################################################
if(X11_Xfixes_FOUND)
  option(FLTK_OPTION_USE_XFIXES "use lib Xfixes" ON)
endif(X11_Xfixes_FOUND)

if(FLTK_OPTION_USE_XFIXES)
...
endif(FLTK_OPTION_USE_XFIXES)

#######################################################################
if(X11_Xcursor_FOUND)
  option(FLTK_OPTION_USE_XCURSOR "use lib Xcursor" ON)
endif(X11_Xcursor_FOUND)

if(FLTK_OPTION_USE_XCURSOR)
...
endif(FLTK_OPTION_USE_XCURSOR)

#######################################################################
if(X11_Xft_FOUND)
  option(FLTK_OPTION_USE_XFT "use lib Xft" ON)
  option(FLTK_OPTION_USE_PANGO "use lib Pango" OFF)
  if(NOT FLTK_OPTION_WAYLAND)
    option(FLTK_OPTION_USE_CAIRO "all drawing to X11 windows uses Cairo" OFF)
  endif(NOT FLTK_OPTION_WAYLAND)
endif(X11_Xft_FOUND)
...
if(FLTK_OPTION_USE_PANGO OR FLTK_OPTION_USE_CAIRO)
  if(FLTK_OPTION_WAYLAND OR FLTK_APPLE_X11)
    set(USE_PANGOXFT false)
  else()
    set(USE_PANGOXFT true)
  endif(FLTK_OPTION_WAYLAND OR FLTK_APPLE_X11)
endif(FLTK_OPTION_USE_PANGO OR FLTK_OPTION_USE_CAIRO)
...
if(USE_PANGOXFT)
  if(NOT X11_Xft_FOUND)
    ...
    message(STATUS "Please install Xft development files and try again or disable FLTK_OPTION_USE_PANGO.")
    message(FATAL_ERROR "*** Aborting ***")
  else()
    if(NOT FLTK_OPTION_USE_XFT)
      message(STATUS "Pango requires Xft but usage of Xft was disabled.")
      message(STATUS "Please enable FLTK_OPTION_USE_XFT and try again or disable FLTK_OPTION_USE_PANGO.")
      message(FATAL_ERROR "*** Aborting ***")
    endif(NOT FLTK_OPTION_USE_XFT)
  endif(NOT X11_Xft_FOUND)
endif(USE_PANGOXFT)

#######################################################################
if((X11_Xft_FOUND OR NOT USE_PANGOXFT) AND FLTK_OPTION_USE_PANGO)
  ...
  if((PANGOXFT_FOUND OR NOT USE_PANGOXFT) AND PANGOCAIRO_FOUND AND CAIRO_FOUND)
    ...
    endif(APPLE AND FLTK_APPLE_X11)
  ...
  if(USE_PANGO AND (FLTK_OPTION_USE_CAIRO OR FLTK_OPTION_WAYLAND))
    set(FLTK_USE_CAIRO 1)
    # fl_debug_var(FLTK_USE_CAIRO)
  endif(USE_PANGO AND (FLTK_OPTION_USE_CAIRO OR FLTK_OPTION_WAYLAND))

endif((X11_Xft_FOUND OR NOT USE_PANGOXFT) AND FLTK_OPTION_USE_PANGO)

if(FLTK_OPTION_WAYLAND)

  # Note: Disable FLTK_OPTION_WAYLAND_GTK to get cairo titlebars rather than GTK
  if(FLTK_OPTION_WAYLAND_GTK)
  ...
  endif(FLTK_OPTION_WAYLAND_GTK)

endif(FLTK_OPTION_WAYLAND)

if(FLTK_OPTION_USE_XFT)
...
endif(FLTK_OPTION_USE_XFT)

#######################################################################
if(X11_Xrender_FOUND)
  option(FLTK_OPTION_USE_XRENDER "use lib Xrender" ON)
endif(X11_Xrender_FOUND)

if(FLTK_OPTION_USE_XRENDER)
...
else(FLTK_OPTION_USE_XRENDER)
  set(FLTK_XRENDER_FOUND FALSE)
endif(FLTK_OPTION_USE_XRENDER)
...
#######################################################################
option(FLTK_OPTION_USE_KDIALOG "Fl_Native_File_Chooser may run kdialog" ON)
if(FLTK_OPTION_USE_KDIALOG)
  set(USE_KDIALOG 1)
else()
  set(USE_KDIALOG 0)
endif(FLTK_OPTION_USE_KDIALOG)
#######################################################################
